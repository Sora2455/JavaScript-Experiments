 ...
 * @see <a href="http://jeromeetienne.github.com/jquery-qrcode/" target="_blank">
 * http://jeromeetienne.github.com/jquery-qrcode/</a>
 */
// ---------------------------------------------------------------------
...
class QR8bitByte {
    ...
    constructor(data: string) {
        ...
        for (let i = 0, l = this.data.length; i < l; i++) {
            ...
            tempData.push(byteArray);
        }

        this.parsedData = Array.prototype.concat.apply([], tempData);
...
export class QRCodeModel {
    ...
    private static createData(typeNumber: number, errorCorrectLevel: QRErrorCorrectLevel,
                              dataList: QR8bitByte[]): number[] {
    ...
    private static createBytes(buffer: QRBitBuffer, rsBlocks: QRRSBlock[]): number[] {
        ...
        let maxDcCount = 0;
        let maxEcCount = 0;
        const dcdata = new Array(rsBlocks.length) as number[][];
        const ecdata = new Array(rsBlocks.length) as number[][];
        ...
        const data = new Array(totalCodeCount) as number[];
        let index = 0;
...
class QRUtil {
    ...
    public static getMask(maskPattern: QRMaskPattern, i: number, j: number): boolean {
        switch (maskPattern) {
        ...
        for (let row = 0; row < moduleCount; row++) {
            for (let col = 0; col < moduleCount - 6; col++) {
                if (qrCode.isDark(row, col) && !qrCode.isDark(row, col + 1) &&
                    ...
                    qrCode.isDark(row, col + 4) && !qrCode.isDark(row, col + 5) && qrCode.isDark(row, col + 6)) {
                    lostPoint += 40;
                }
            }
        }
        for (let col = 0; col < moduleCount; col++) {
            for (let row = 0; row < moduleCount - 6; row++) {
                if (qrCode.isDark(row, col) && !qrCode.isDark(row + 1, col) &&
                    ...
                    qrCode.isDark(row + 4, col) && !qrCode.isDark(row + 5, col) && qrCode.isDark(row + 6, col)) {
                    lostPoint += 40;
                }
            }
        }
...
class QRMath {
    public static EXP_TABLE = new Array(256) as number[];
    public static LOG_TABLE = new Array(256) as number[];
...
class QRRSBlock {
    public static getRSBlocks(typeNumber: number, errorCorrectLevel: QRErrorCorrectLevel): QRRSBlock[] {
 ...
 * @private
 * @param {String} sText
 * @param {QRErrorCorrectLevel} nCorrectLevel
 * @return {Number} type
 */
export function _getTypeNumber(sText: string, nCorrectLevel: QRErrorCorrectLevel): number {
    let nType = 1;
