...
for %%i in (*) do start cmd.exe @cmd /k "emcc %%~nxi -O3 -s INVOKE_RUN=0 -s ENVIRONMENT='worker' -s WASM=0 -s MINIMAL_RUNTIME=1 -s MIN_IE_VERSION=10 -o ../../build/bytecode/%%~ni.asm.js && exit || echo error in %%~nxi"
