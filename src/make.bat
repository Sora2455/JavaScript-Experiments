...
for %%i in (*) do start cmd.exe @cmd /k "emcc %%~nxi -O0 -s INVOKE_RUN=0 -s ENVIRONMENT='worker' -s WASM=1 -s MINIMAL_RUNTIME=1 -o ../../build/bytecode/%%~ni.wasm && exit || echo error in %%~nxi"
